...
The OAuth2 token is requested from a different host, so `token_timeout` bounds it on
its own. It follows `timeout` when it is not given, and `None` leaves it unbounded.

```python
# The token request gives up sooner than the API request
api = AmazonCreatorsApi(
    ID, SECRET, VERSION, TAG, COUNTRY, timeout=(1, 10), token_timeout=(0.5, 2)
)
```
