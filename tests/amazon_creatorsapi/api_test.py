...
class TestAmazonCreatorsApiItems(unittest.TestCase):
    ...
    @mock.patch("amazon_creatorsapi.api.ApiClient")
    def test_token_timeout_follows_the_timeout(
        self,
        mock_client_class: MagicMock,
    ) -> None:
        """Test that the token request follows the request timeout.

        None is how a client asks for an unbounded request, and the token
        request follows it there too.
        """
        for timeout in (12.0, (1.0, 12.0), None):
            with self.subTest(timeout=timeout):
                mock_client_class.return_value = MagicMock()

                api = self.build_api_with_timeouts(timeout=timeout)

                self.assertEqual(api.timeout, timeout)
                self.assertEqual(api.token_timeout, timeout)
    ...
    def build_api_with_timeouts(
        ...
        """Build an API client with the given timeouts."""
        return AmazonCreatorsApi(
            credential_id=self.credential_id,
            credential_secret=self.credential_secret,
            version=self.version,
