...
class TestBuildHttpxTimeout(unittest.TestCase):
    ...
    def test_pair_bounds_connect_and_read_apart(self) -> None:
        """Test that a pair bounds the connect and the read leg on its own."""
        self.assertEqual(
            build_httpx_timeout((1, 5)),
            httpx.Timeout(connect=1, read=5, write=5, pool=5),
        )

    def test_pair_leaves_no_leg_unbounded(self) -> None:
        """Test that every leg of a pair is bounded.
